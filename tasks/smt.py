...
def do_smt(catalog):
    ...
    for tr in pbar(trs, task_str):
        ...
        if not cols:
            continue
        name = 'AT' + cols[0]
        name, source = catalog.new_entry(name, name='SMT', url=smt_url)
        catalog.entries[name].add_quantity(SUPERNOVA.ALIAS, cols[1], source)
        catalog.entries[name].add_quantity(SUPERNOVA.CLAIMED_TYPE, cols[2], source)
        catalog.entries[name].add_quantity(SUPERNOVA.RA, cols[3], source, u_value='floatdegrees')
        catalog.entries[name].add_quantity(SUPERNOVA.DEC, cols[4], source, u_value='floatdegrees')
