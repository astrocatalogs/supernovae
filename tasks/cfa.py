...
def do_cfa_photo(catalog):
    ...
    for fname in pbar(file_names, task_str, sort=True):
        ...
        catalog.entries[name].add_quantity(SUPERNOVA.DISCOVER_DATE, year, secondarysource)
    ...
    # Hicken 2012
    path = os.path.join(catalog.get_current_task_repo(), 'hicken-2012-standard.dat')
    ...
    # Bianco 2014
    path = os.path.join(catalog.get_current_task_repo(), 'bianco-2014-standard.dat')
    with open(path, 'r') as infile:
        tsvin = list(csv.reader(infile, delimiter=' ', skipinitialspace=True))
        for row in pbar(tsvin, task_str):
            ...
            source = catalog.entries[name].add_source(bibcode='2014ApJS..213...19B')
            catalog.entries[name].add_quantity(SUPERNOVA.ALIAS, name, source)
            photodict = {
                PHOTOMETRY.U_TIME: 'MJD',
...
def do_cfa_spectra(catalog):
    ...
    # II spectra
    oldname = ''
    file_names = next(os.walk(os.path.join(catalog.get_current_task_repo(), 'CfA_SNII')))[1]
    for ni, name in enumerate(pbar(file_names, task_str, sort=True)):
        fullpath = os.path.join(catalog.get_current_task_repo(), 'CfA_SNII/') + name
        origname = name
        if name.startswith('sn') and is_number(name[2:6]):
            name = 'SN' + name[2:]
        # Use existing entry name if it already exists
        ...
        source = catalog.entries[name].add_source(
            name=reference, url=refurl, secondary=True, acknowledgment=ACKN_CFA)
        catalog.entries[name].add_quantity(SUPERNOVA.ALIAS, name, source)
        for fi, fname in enumerate(sorted(glob(fullpath + '/*'), key=lambda s: s.lower())):
            ...
            errors = data[2]
            sources = uniq_cdl(
                [source, catalog.entries[name].add_source(bibcode='2017arXiv170601030H')]
            )
    ...
    # Ia spectra
    oldname = ''
    file_names = next(os.walk(os.path.join(catalog.get_current_task_repo(), 'CfA_SNIa')))[1]
    for ni, name in enumerate(pbar(file_names, task_str, sort=True)):
        ...
        if name.startswith('snf') and is_number(name[3:7]):
            name = 'SNF' + name[3:]
        # Use existing entry name if it exists
        _name = catalog.get_name_for_entry_or_alias(name)
        if _name is not None:
            name = _name
        if oldname and name != oldname:
        ...
        for fi, fname in enumerate(sorted(glob(fullpath + '/*'), key=lambda s: s.lower())):
            ...
            if origname.startswith('sn') and is_number(origname[2:6]):
                ...
                instrument = fileparts[3].split('.')[0]
            time = str(
                astrotime(year + '-' + month + '-' + str(floor(float(day))).zfill(2)).mjd +
            ...
            sources = uniq_cdl(
                [source, catalog.entries[name].add_source(bibcode='2012AJ....143..126B'),
                 catalog.entries[name].add_source(bibcode='2008AJ....135.1598M')]
            )
            catalog.entries[name].add_spectrum(
                u_wavelengths='Angstrom',
                u_fluxes='erg/s/cm^2/Angstrom',
    ...
    # Ibc spectra
    oldname = ''
    file_names = next(os.walk(os.path.join(catalog.get_current_task_repo(), 'CfA_SNIbc')))[1]
    for ni, name in enumerate(pbar(file_names, task_str)):
        fullpath = os.path.join(catalog.get_current_task_repo(), 'CfA_SNIbc/') + name
        if name.startswith('sn') and is_number(name[2:6]):
            name = 'SN' + name[2:]
        # Use existing name if entry already added
        _name = catalog.get_name_for_entry_or_alias(name)
        if _name is not None:
            name = _name
        if oldname and name != oldname:
        ...
        for fi, fname in enumerate(sorted(glob(fullpath + '/*'), key=lambda s: s.lower())):
            ...
            fluxes = data[1]
            sources = uniq_cdl(
                [source, catalog.entries[name].add_source(bibcode='2014AJ....147...99M')]
            )
            spec = {
                ...
                SPECTRUM.DEREDDENED: False,
                SPECTRUM.DEREDSHIFTED: False
            }
            if time:
    ...
    # Other spectra
    oldname = ''
    file_names = next(os.walk(os.path.join(catalog.get_current_task_repo(), 'CfA_Extra')))[1]
    for ni, name in enumerate(pbar(file_names, task_str, sort=True)):
        fullpath = os.path.join(catalog.get_current_task_repo(), 'CfA_Extra/') + name
        if name.startswith('sn') and is_number(name[2:6]):
            name = 'SN' + name[2:]
        # Use existing name if entry already added
        _name = catalog.get_name_for_entry_or_alias(name)
        ...
        for fi, fname in enumerate(sorted(glob(fullpath + '/*'), key=lambda s: s.lower())):
            ...
            if len(fileparts) > 1:
                ...
                if is_number(year) and is_number(month) and is_number(day):
                    ...
                    time = year + '-' + month + '-' + str(floor(float(day))).zfill(2)
                    time = str(astrotime(time).mjd + float(day) - floor(float(day)))
            f = open(fname, 'r')
            data = csv.reader(f, delimiter=' ', skipinitialspace=True)
            data = [list(i) for i in zip(*data)]
            wavelengths = data[0]
            ...
            spec = {
                ...
                SPECTRUM.FILENAME: filename,
                SPECTRUM.FLUXES: fluxes,
                SPECTRUM.U_TIME: 'MJD' if time else '',
